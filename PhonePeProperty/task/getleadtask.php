<?php 
	...
	if(isset($_REQUEST['userid'])) {
		...
		$query_task = $conn->query("select * from tblstafftasks where rel_type='lead' and rel_id='".$userid."' order by dateadded desc limit 10");

		if($query_task->num_rows != 0){
			
			while($result = $query_task->fetch_assoc())
				{
